        ...
        success: function (response) {
          ...
          function cut_movies_off() {
            ...
            $trailer_link =
              ...
              $link_data +
              "?enablejsapi=1&controls=0&fs=0&iv_load_policy=3&rel=0&showinfo=0&start=1&mute=0&modestbranding=0";
          ...
          if (response == "no-login") {
            ...
            //if received user json data
            $user = JSON.parse(response);

            //if user subscribed to TV plan cut off movies
            if ($user.sub_type == "m1" || $user.sub_type == "y1") {
              cut_movies_off();
          ...
          function getMoviePaths(title, s_title, callback) {
            // console.log(title);
            var posterPath;
            var backdropPath;
            var suggestionPath;
          ...
          $("#reviewSubmit").click(function (event) {
            ...
            request = $.ajax({
              url: "./includes/webflix_store_movie_review.php",
              type: "POST",
              dataType: "text",
